  ...
  for (std::size_t i = 1; i <= p.size(); ++i) {
    if (i == p.size() || p[i] == '/') {
      ...
      if (::mkdir(p.c_str(), 0755) < 0 && errno != EEXIST) {
        std::fprintf(stderr, "sysprop-init: mkdir('%s'): %s\n", p.c_str(), std::strerror(errno)); // NOLINT(cppcoreguidelines-pro-type-vararg)
        p[i] = saved;
        return false;
      }
      p[i] = saved;
...
void CleanupTmpFiles(const char* dir) {
  ...
  while ((entry = ::readdir(d)) != nullptr) {
    if (std::strncmp(entry->d_name, ".tmp.", 5) != 0) { continue; } // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)

    char path[4096];
    std::snprintf(path, sizeof(path), "%s/%s", dir, entry->d_name); // NOLINT(cppcoreguidelines-pro-type-vararg,cppcoreguidelines-pro-bounds-array-to-pointer-decay)
...
InitArgs ParseInitArgs(int argc, char* argv[]) {
  InitArgs args;
  if (const char* rd = std::getenv("SYSPROP_RUNTIME_DIR"))    { args.runtime_dir    = rd; } // NOLINT(concurrency-mt-unsafe)
  if (const char* pd = std::getenv("SYSPROP_PERSISTENT_DIR")) { args.persistent_dir = pd; } // NOLINT(concurrency-mt-unsafe)
