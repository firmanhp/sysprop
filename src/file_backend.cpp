...
UniqueFd OpenWriteNew(const char* path) {
  return UniqueFd{::open(path, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644)};
}
...
FileBackend::FileBackend(std::string base_path) : base_path_(std::move(base_path)) {}

bool FileBackend::BuildPath(char* dst, std::size_t dst_len, const char* key) const noexcept {
  const int n = std::snprintf(dst, dst_len, "%s/%s", base_path_.c_str(), key);
...
int FileBackend::Set(const char* key, const char* value) {
  ...
  const int n = std::snprintf(tmp_path, sizeof(tmp_path), "%s/.tmp.%s.%d", base_path_.c_str(), key,
...
int FileBackend::ForEach(Visitor visitor) {
  UniqueDir dir = OpenDir(base_path_.c_str());
