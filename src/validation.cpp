...
int ValidateKey(std::string_view key) noexcept {
  if (key.empty()) return SYSPROP_ERR_INVALID_KEY;
  if (key.size() >= SYSPROP_MAX_KEY_LENGTH) return SYSPROP_ERR_KEY_TOO_LONG;

  // Must not start or end with a dot.
  if (key.front() == '.' || key.back() == '.') return SYSPROP_ERR_INVALID_KEY;
  ...
  for (const unsigned char c : key) {
    ...
    if (!is_allowed) return SYSPROP_ERR_INVALID_KEY;

    // No consecutive dots (empty segment between dots).
    if (c == '.' && prev_dot) return SYSPROP_ERR_INVALID_KEY;
    prev_dot = (c == '.');
  }
